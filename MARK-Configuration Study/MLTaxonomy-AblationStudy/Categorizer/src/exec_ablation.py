...
def ablation(input_path=os.path.join(script_dir, "../","../", "repos"),output_path=script_dir):
    """
      Perform ablation analysis on ML model producers and consumers using specified input and output paths.

      Parameters:
          input_path (str): The base directory containing input data (default is '../../repos' relative to the script).
          output_path (str): The base directory to store output data (default is the script directory).

      Raises:
          FileNotFoundError: If any required dictionary or input folder is missing.
      """
  ...
  # Get the directory of the current script
    ...
    output_folders = [
        os.path.join(output_base_folder, "Producers_1"),
        os.path.join(output_base_folder, "Producers_2")
    ]
    producer_dict_paths = [
        os.path.join(script_dir, "library_dictionary", "library_dict_producers_1.csv"),
    ...
    output_folders = [
        os.path.join(output_base_folder, "Consumers_1"),
        os.path.join(output_base_folder, "Consumers_2"),
        os.path.join(output_base_folder, "Consumers_3"),
        os.path.join(output_base_folder, "Consumers_4"),
        os.path.join(output_base_folder, "Consumers_5")
    ]
    consumer_dict_paths = [
        os.path.join(script_dir, "library_dictionary", "library_dict_consumers_1.csv"),
        os.path.join(script_dir, "library_dictionary", "library_dict_consumers_2.csv")
    ]

    # Initialize a counter for iterations
    iteration_counter = 1
    # Analyze the ML-Model Consumer projects for each library dictionary and corresponding output folder
    for output_folder in output_folders:
        # Determine the value of rules_3
        if iteration_counter == 1 or iteration_counter == 2 or iteration_counter == 4:
            rules_3 = False
        else:
            rules_3 = True
        ...
        if iteration_counter == 1:
            ...
            current_producer_dict_path = producer_dict_paths[1]
            current_consumer_dict_path = consumer_dict_paths[1]
...
if __name__ == "__main__":
    ...
    ablation(input_path=args.input_path, output_path=args.output_path)